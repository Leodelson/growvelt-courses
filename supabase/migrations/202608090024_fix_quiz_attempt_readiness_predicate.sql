-- Correct the valid-quiz readiness predicate introduced in migration 023.
begin;
...
begin
  if not exists (
    ...
    raise exception 'Quiz readiness correction aborted: expected hardened submit RPC baseline is missing or insecure';
  end if;
end;
$$;
...
begin
  if not exists (
    ...
    raise exception 'Quiz readiness correction aborted: submit RPC security or grants are not hardened';
  end if;
  ...
  for mismatch_row in
    ...
    limit 1
  loop
    raise exception 'Quiz readiness correction aborted: submit RPC return contract mismatch'
      using detail = format(
