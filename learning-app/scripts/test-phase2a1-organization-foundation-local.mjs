...
const probe = await run(["exec","-i",name,"psql","-X","-A","-t","-U","postgres","-d","postgres"],"select to_regclass('public.learning_provider_organizations') is not null;");
if (probe !== "t") await run(["exec","-i",name,"psql","-X","-v","ON_ERROR_STOP=1","-U","postgres","-d","postgres"],await readFile(path.join(root,"supabase","migrations","20260919000000_add_training_organization_provider_foundation.sql"),"utf8"));
await run(["exec","-i",name,"psql","-X","-v","ON_ERROR_STOP=1","-U","postgres","-d","postgres"],await readFile(path.join(root,"supabase","tests","phase2a1_training_organization_foundation.sql"),"utf8"));
console.log("PASS Phase 2A1 training-organization foundation on isolated local Supabase");
