...
await run(["exec", "-i", name, "psql", "-X", "-v", "ON_ERROR_STOP=1", "-U", "postgres", "-d", "postgres"], await readFile(path.join(root, "supabase", "tests", "phase0d_account_deletion_retention.sql"), "utf8"));
console.log("PASS account deletion and certificate retention on isolated local Supabase");
