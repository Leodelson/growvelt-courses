...
const probe = await run(["exec", "-i", name, "psql", "-X", "-A", "-t", "-U", "postgres", "-d", "postgres"], "select to_regclass('public.learning_company_paid_course_purchases') is not null;");
if (probe === "t") await run(["exec", "-i", name, "psql", "-X", "-v", "ON_ERROR_STOP=1", "-U", "postgres", "-d", "postgres"], "drop table public.learning_company_paid_course_purchase_seats, public.learning_company_paid_course_purchases cascade;");
await run(["exec", "-i", name, "psql", "-X", "-v", "ON_ERROR_STOP=1", "-U", "postgres", "-d", "postgres"], await readFile(path.join(databaseRoot, "migrations", "20260938000000_add_company_paid_course_purchase_foundation.sql"), "utf8"));
await run(["exec", "-i", name, "psql", "-X", "-v", "ON_ERROR_STOP=1", "-U", "postgres", "-d", "postgres"], await readFile(path.join(databaseRoot, "tests", "phase3c1_company_paid_course_purchase_foundation.sql"), "utf8"));
console.log("PASS Phase 3C1 company paid-course purchase foundation on isolated local Supabase");
