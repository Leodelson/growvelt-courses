...
await run(["exec", "-i", name, "psql", "-X", "-v", "ON_ERROR_STOP=1", "-U", "postgres", "-d", "postgres"], "drop table if exists public.learning_company_paid_course_purchase_attempts, public.learning_company_paid_course_purchase_seats, public.learning_company_paid_course_purchases cascade; drop function if exists public.start_learning_company_paid_course_checkout(bigint), public.grant_paid_learning_company_purchase_access(bigint), public.mark_learning_company_paid_course_checkout_pending(text), public.fail_learning_company_paid_course_checkout(text,text,text), public.finalize_learning_company_paid_course_purchase(bigint,text,text), public.finalize_learning_company_paid_course_purchase_by_reference(text,text), public.finalize_learning_company_paid_course_purchase_by_reference(text,text,bigint,text,text) cascade;");
for (const filename of ["20260938000000_add_company_paid_course_purchase_foundation.sql", "20260939000000_add_company_paid_course_access_boundary.sql", "20260940000000_add_company_paystack_test_checkout.sql"]) {
