...
const probe = await run(["exec","-i",name,"psql","-X","-A","-t","-U","postgres","-d","postgres"],"select to_regclass('public.learning_company_course_assignments') is not null;");
if (probe !== "t") await run(["exec","-i",name,"psql","-X","-v","ON_ERROR_STOP=1","-U","postgres","-d","postgres"],await readFile(path.join(root,"supabase","migrations","20260937000000_add_company_course_assignments.sql"),"utf8"));
await run(["exec","-i",name,"psql","-X","-v","ON_ERROR_STOP=1","-U","postgres","-d","postgres"],await readFile(path.join(root,"supabase","tests","phase3b1_company_course_assignments.sql"),"utf8"));
console.log("PASS Phase 3B1 company course assignments on isolated local Supabase");
