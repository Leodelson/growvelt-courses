...
const probe = await run(["exec", "-i", name, "psql", "-X", "-A", "-t", "-U", "postgres", "-d", "postgres"], "select to_regprocedure('public.get_own_learning_provider_organization_profile_branding(bigint)') is not null;");
if (probe !== "t") throw new Error("Apply the Phase 2B3 branding migration to the isolated local Supabase database before running this test.");
await run(["exec", "-i", name, "psql", "-X", "-v", "ON_ERROR_STOP=1", "-U", "postgres", "-d", "postgres"], await readFile(path.join(root, "supabase", "tests", "phase2b3_provider_branding.sql"), "utf8"));
console.log("PASS Phase 2B3 private provider branding and verified public profile on isolated local Supabase");
