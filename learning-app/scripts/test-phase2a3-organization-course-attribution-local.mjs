...
for (const migration of ["20260919000000_add_training_organization_provider_foundation.sql", "20260920000000_add_training_organization_invitation_foundation.sql", "20260921000000_add_training_organization_course_attribution.sql"]) {
  const probe = migration.startsWith("202609210") ? "select exists(select 1 from information_schema.columns where table_schema='public' and table_name='learning_courses' and column_name='organization_id');" : migration.startsWith("202609200") ? "select to_regclass('public.learning_provider_organization_invitations') is not null;" : "select to_regclass('public.learning_provider_organizations') is not null;";
