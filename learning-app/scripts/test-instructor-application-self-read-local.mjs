...
const probe = "select to_regprocedure('public.get_own_instructor_application()') is not null;";
if ((await run(["exec", "-i", name, "psql", "-X", "-A", "-t", "-U", "postgres", "-d", "postgres"], probe)) !== "t") await run(["exec", "-i", name, "psql", "-X", "-v", "ON_ERROR_STOP=1", "-U", "postgres", "-d", "postgres"], await readFile(path.join(root, "supabase", "migrations", "20260923000000_add_own_instructor_application_read.sql"), "utf8"));
