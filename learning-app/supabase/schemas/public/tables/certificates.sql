create table "public"."certificates" (
  ...
  constraint "certificates_retention_state_check" check ((retention_state = ANY (ARRAY['account_active'::text, 'preserved_after_account_deletion'::text, 'anonymized_after_account_deletion'::text]))),
  constraint "certificates_retention_lifecycle_check" check ((((retention_state = 'account_active'::text) AND (learner_id IS NOT NULL) AND (account_deleted_at IS NULL)) OR ((retention_state = 'preserved_after_account_deletion'::text) AND (learner_id IS NULL) AND (learner_name IS NOT NULL) AND (account_deleted_at IS NOT NULL)) OR ((retention_state = 'anonymized_after_account_deletion'::text) AND (learner_id IS NULL) AND (learner_name IS NULL) AND (account_deleted_at IS NOT NULL) AND (status = 'revoked'::text)))),
