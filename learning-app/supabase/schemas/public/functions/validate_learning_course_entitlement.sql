...
begin
  ...
  if not found or order_row.status <> 'paid' or order_row.learner_id is distinct from new.learner_id or order_row.course_id is distinct from new.course_id
  then raise exception 'Paid entitlement must match a paid authoritative order' using errcode = '22023'; end if;
  return new;
end;
$function$;
revoke all on function public.validate_learning_course_entitlement() from public, anon, authenticated;
