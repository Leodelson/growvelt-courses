create or replace function public.get_own_enrolled_lesson_snapshot (
  p_slug      text,
  p_lesson_id bigint
)
  returns table (
    ...
    current_video_provider   text,
    current_video_reference  text,
    current_video_visibility text,
    current_duration_seconds integer,
    previous_lesson_id       bigint,
...
grant execute on function "public"."get_own_enrolled_lesson_snapshot"(text, bigint) to "authenticated", "postgres", "service_role";

revoke all on function "public"."get_own_enrolled_lesson_snapshot"(text, bigint) from public;
