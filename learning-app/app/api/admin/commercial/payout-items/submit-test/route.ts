...
import { submitInstructorTestPayout } from "@/app/lib/admin/instructor-payout-transfer";
...
export async function POST(request: Request) {
  ...
  try { await submitInstructorTestPayout(Number(body.payoutItemId), user.id); return NextResponse.json({ status: "pending" }); }
  catch { return NextResponse.json({ code: "transfer_unavailable" }, { status: 409 }); }
}
