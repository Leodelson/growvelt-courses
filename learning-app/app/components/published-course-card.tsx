...
export function PublishedCourseCard({ course, index }: { course: PublishedCourse; index: number }) {
  ...
  return <article className="course-card published-course-card">
    ...
    <div className="course-card-body">
      ...
      <h2>{course.title}</h2><p>{course.summary || "A practical Growvelt Learning course."}</p>
      ...
      <div className="course-card-footer"><strong>{course.isFree ? "Open to learn" : pricing}</strong><Link href={`/dashboard/courses/${encodeURIComponent(course.slug)}`}>View course</Link></div>
    </div>
  </article>;
}
