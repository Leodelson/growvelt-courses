...
export async function LearningShell({ children }: { children: React.ReactNode }) {
  ...
  return <LearningShellNavigation initialSidebarCollapsed={initialSidebarCollapsed} isInstructor={isInstructor} isAdmin={isAdmin} userEmail={email} displayName={displayName} avatarUrl={profile?.avatarUrl ?? null}>{children}</LearningShellNavigation>;
}
