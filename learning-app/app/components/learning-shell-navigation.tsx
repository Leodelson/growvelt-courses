...
function MobileWorkspaceLinks({ isAdmin, isInstructor, isCompany, onNavigate }: Pick<WorkspaceAccess, "isAdmin" | "isInstructor" | "isCompany"> & { onNavigate: () => void }) {
  ...
  return <div className="mobile-dashboard-workspaces">
    <section><p className="eyebrow">Learning</p>{navigation.filter((item) => item.href !== "/dashboard" && !item.external && (item.href !== "/dashboard/company" || isCompany)).map((item) => <Link onClick={onNavigate} href={item.href} key={item.href}><LearningIcon name={item.icon} />{t(item.labelKey)}</Link>)}{isInstructor && <Link onClick={onNavigate} href="/dashboard/instructor/organizations"><LearningIcon name="organization" />Organizations</Link>}</section>
    {isInstructor && <section><p className="eyebrow">Instructor workspace</p><Link onClick={onNavigate} href="/dashboard/instructor"><LearningIcon name="instructor-review" />{t("nav.instructor")}</Link><Link onClick={onNavigate} href="/dashboard/instructor/payout-profile"><LearningIcon name="payout" />Payout profile</Link></section>}
