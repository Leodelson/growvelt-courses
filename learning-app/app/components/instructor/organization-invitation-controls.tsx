...
export function OrganizationInvitationForm({ organizationId }: { organizationId: number }) {
  ...
  return <form className="organization-invitation-form" onSubmit={submit}><p className="form-required-key"><span className="required-mark" aria-hidden="true">*</span> Required fields</p><label className="course-field">Approved instructor email <span className="required-mark" aria-hidden="true">*</span><span className="sr-only"> (required)</span><input name="email" type="email" placeholder="instructor@example.com" maxLength={320} required /><span>The person must already have an approved Growvelt instructor account. They will receive an email and can accept the invitation from their Organizations page.</span></label><label className="course-field">Organization role <span className="required-mark" aria-hidden="true">*</span><span className="sr-only"> (required)</span><select name="role" defaultValue="instructor" required><option value="instructor">Instructor</option><option value="admin">Admin</option></select><span>Admins can help organize the provider workspace. This phase does not give either role control over another instructor’s personal earnings.</span></label><button className="button button-secondary" disabled={pending}>{pending ? "Inviting…" : "Invite instructor"}</button>{feedback && <p className={`payout-profile-feedback${feedback.kind === "error" ? " is-error" : ""}`} role="status">{feedback.text}</p>}</form>;
}
