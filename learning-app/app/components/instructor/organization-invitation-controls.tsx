...
export function OrganizationInvitationAcceptance({ invitationId }: { invitationId: number }) {
  ...
  async function respond(kind: "accept" | "decline") { if (pending) return; if (kind === "decline" && !window.confirm("Decline this organization invitation?")) return; setPending(true); setMessage(null); try { const response = await fetch(`/api/instructor/organization-invitations/${invitationId}/${kind}`, { method: "POST", credentials: "same-origin" }); const result = await response.json().catch(() => null) as { notification?: "sent" | "not_configured" | "failed" } | null; if (!response.ok) throw new Error("unavailable"); setMessage(kind === "accept" ? result?.notification === "sent" ? "Organization invitation accepted. The organization owner has been notified by email." : "Organization invitation accepted." : "Organization invitation declined."); router.refresh(); } catch { setMessage(kind === "accept" ? "We could not accept this invitation safely. Refresh and try again." : "We could not decline this invitation safely. Refresh and try again."); } finally { setPending(false); } }
  return <div className="organization-invitation-acceptance"><div className="organization-action-buttons"><button className="button button-primary" type="button" onClick={() => respond("accept")} disabled={pending}>{pending ? "Responding…" : "Accept invitation"}</button><button className="button button-secondary" type="button" onClick={() => respond("decline")} disabled={pending}>Decline invitation</button></div>{message && <p className="payout-profile-feedback" role="status">{message}</p>}</div>;
}

export function OrganizationInvitationCancellation({ organizationId, invitationId }: { organizationId: number; invitationId: number }) {
  const router = useRouter(); const [pending, setPending] = useState(false); const [message, setMessage] = useState<string | null>(null);
  async function cancel() { if (pending || !window.confirm("Cancel this pending invitation? The instructor will no longer be able to accept it.")) return; setPending(true); setMessage(null); try { const response = await fetch(`/api/instructor/organizations/${organizationId}/invitations/${invitationId}/cancel`, { method: "POST", credentials: "same-origin" }); if (!response.ok) throw new Error("unavailable"); setMessage("Invitation cancelled."); router.refresh(); } catch { setMessage("We could not cancel this invitation. Refresh and try again."); } finally { setPending(false); } }
  return <div className="organization-management-action"><button className="text-button text-button-danger" type="button" onClick={cancel} disabled={pending}>{pending ? "Cancelling…" : "Cancel invitation"}</button>{message && <small role="status">{message}</small>}</div>;
}
...
export function OrganizationMemberRevocation({ organizationId, memberId, memberName }: { organizationId: number; memberId: string; memberName: string }) {
  const router = useRouter(); const [pending, setPending] = useState(false); const [message, setMessage] = useState<string | null>(null);
  async function revoke() { if (pending || !window.confirm(`Remove ${memberName} from this organization? This removes organization access only; it does not delete their individual courses or earnings.`)) return; setPending(true); setMessage(null); try { const response = await fetch(`/api/instructor/organizations/${organizationId}/members/${memberId}/revoke`, { method: "POST", credentials: "same-origin", headers: { "Content-Type": "application/json" }, body: JSON.stringify({}) }); if (!response.ok) throw new Error("unavailable"); setMessage("Member removed from this organization."); router.refresh(); } catch { setMessage("We could not remove this member. Refresh and try again."); } finally { setPending(false); } }
  return <div className="organization-management-action"><button className="text-button text-button-danger" type="button" onClick={revoke} disabled={pending}>{pending ? "Removing…" : "Remove member"}</button>{message && <small role="status">{message}</small>}</div>;
}
