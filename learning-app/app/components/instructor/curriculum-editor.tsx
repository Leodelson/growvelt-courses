...
function QuizBuilder({ lessonId, editable }: { lessonId: number; editable: boolean }) {
  ...
  async function saveQuestion(event: FormEvent<HTMLFormElement>, questionId?: number) { event.preventDefault(); const form = new FormData(event.currentTarget); const options = [0, 1, 2, 3, 4, 5].map((index) => String(form.get(`option-${index}`) ?? "").trim()).filter(Boolean); const correct = Number(form.get("correct-option")); const question = String(form.get("question") ?? "").trim(); if (question.length < 2 || options.length < 2 || options.length > 6 || !Number.isInteger(correct) || correct < 0 || correct >= options.length) { setError("Add a question, 2–6 options, and select exactly one correct option before saving."); return; } setPending(true); setError(null); const { data, error: rpcError } = await createClient().rpc("upsert_instructor_quiz_question", { p_lesson_id: lessonId, p_question_id: questionId ?? null, p_question_text: question, p_options: options.map((option, index) => ({ option_text: option, is_correct: index === correct })) }); setPending(false); if (rpcError || !data?.[0]) { if (process.env.NODE_ENV === "development" && rpcError) console.error("Quiz question save failed", { code: rpcError.code, message: rpcError.message, details: rpcError.details, hint: rpcError.hint }); setError("We couldn’t save this question. It has not been added to the course yet."); return; } setRevision((value) => value + 1); }
  ...
  return <section className="quiz-builder" aria-label="Quiz builder"><h3>Quiz builder</h3>{error && <InlineFeedback variant="error">{error}</InlineFeedback>}<form onSubmit={saveConfig} className="quiz-builder-config"><label className="course-field">Instructions<textarea name="instructions" defaultValue={rows?.[0]?.instructions ?? ""} rows={3} maxLength={2000} disabled={!editable || pending} /></label><label className="course-field">Passing percentage<input name="passing-percentage" type="number" min="1" max="100" defaultValue={rows?.[0]?.passing_percentage ?? 70} disabled={!editable || pending} required /></label><ActionButton className="button button-small" type="submit" isPending={pending} pendingLabel="Saving…" disabled={!editable}>Save quiz settings</ActionButton></form>{rows === null ? <p>Loading quiz questions…</p> : !quizId || grouped.size === 0 ? <p className="published-outline-empty">No saved questions yet. Add and save your first question before submitting this course for review.</p> : <ol className="quiz-builder-questions">{[...grouped.entries()].map(([questionId, questionRows], index) => { const first = questionRows[0]; return <li key={questionId}><form onSubmit={(event) => saveQuestion(event, questionId)}><label className="course-field">Question {index + 1}<textarea name="question" defaultValue={first.question_text ?? ""} rows={2} required disabled={!editable || pending} /></label>{questionRows.map((row, optionIndex) => <label className="quiz-option" key={row.option_id}><input type="radio" name="correct-option" value={optionIndex} defaultChecked={row.is_correct ?? false} disabled={!editable || pending} /><input name={`option-${optionIndex}`} defaultValue={row.option_text ?? ""} required disabled={!editable || pending} /></label>)}<div className="row-actions"><ActionButton className="button button-small" type="submit" isPending={pending} pendingLabel="Saving…" disabled={!editable}>Save question</ActionButton><button type="button" className="text-button" disabled={!editable || pending || index === 0} onClick={() => questionAction("move_instructor_quiz_question", { p_lesson_id: lessonId, p_question_id: questionId, p_direction: "up" })}>Move up</button><button type="button" className="text-button" disabled={!editable || pending || index === grouped.size - 1} onClick={() => questionAction("move_instructor_quiz_question", { p_lesson_id: lessonId, p_question_id: questionId, p_direction: "down" })}>Move down</button><button type="button" className="text-button text-button-danger" disabled={!editable || pending} onClick={() => questionAction("delete_instructor_quiz_question", { p_lesson_id: lessonId, p_question_id: questionId })}>Delete</button></div></form></li>; })}</ol>}<form onSubmit={(event) => saveQuestion(event)} className="quiz-builder-add"><h4>Add question</h4><label className="course-field">Question<textarea name="question" rows={2} required disabled={!editable || pending} /></label>{[0, 1, 2, 3].map((index) => <label className="quiz-option" key={index}><input type="radio" name="correct-option" value={index} defaultChecked={index === 0} disabled={!editable || pending} /><input name={`option-${index}`} placeholder={`Option ${index + 1}`} required disabled={!editable || pending} /></label>)}<ActionButton className="button button-small" type="submit" isPending={pending} pendingLabel="Adding…" disabled={!editable || !quizId}>Add question</ActionButton>{!quizId && <p>Save quiz settings before adding questions.</p>}</form></section>;
}
