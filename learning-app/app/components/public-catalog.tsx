...
export function PublicCatalog({ catalog, query, authenticated }: { catalog: PublicCatalogResult; query: PublicCatalogQuery; authenticated: boolean }) {
  ...
  const updateQuery = useCallback((changes: Partial<PublicCatalogQuery>) => {
    router.replace(publicCatalogHref({ ...query, page: 1 }, changes), { scroll: false });
  }, [query, router]);
  ...
  return <section className="catalog-page section-shell">
    <header className="catalog-hero"><p className="eyebrow">Explore Growvelt Learning</p><h1>Find a practical course to begin.</h1><p>Browse live published courses from Growvelt Instructors. Free courses can be enrolled in after you sign in; paid-course checkout is not available yet.</p></header>
    ...
    <div className="public-catalog-controls" aria-label="Course catalog filters">
      ...
      <div className="public-catalog-actions"><Link className="button button-secondary" href="/learn">Reset</Link></div>
    </div>

    {catalog.total === 0 ? <section className="course-empty-state"><p className="eyebrow">No matching courses</p><h2>{hasActiveFilters ? "Try a different search or filter." : "Published courses will appear here as they become available."}</h2><p>{hasActiveFilters ? "Clear one or more filters to see the current published catalog." : "Growvelt is preparing practical learning experiences for this catalog."}</p>{hasActiveFilters ? <Link className="button button-secondary" href="/learn">Clear filters</Link> : null}</section> : <section aria-labelledby="public-catalog-title">
      ...
      <div className="public-catalog-load-more" ref={sentinelRef} aria-live="polite">{isLoadingMore ? <p><span className="catalog-loader" aria-hidden="true" />Loading more courses</p> : null}{loadMoreError ? <button className="button button-secondary" type="button" onClick={() => void loadMore()}>Try loading more</button> : null}</div>
    </section>}
  </section>;
}
