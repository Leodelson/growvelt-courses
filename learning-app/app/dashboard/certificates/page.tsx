...
export default async function LearnerCertificatesPage() {
  ...
  if (!certificates.length) return <section className="certificate-space learner-empty-space" aria-labelledby="certificate-space-title"><div><p className="eyebrow">Certificates</p><h1 id="certificate-space-title">Your issued certificates will appear here.</h1><p>Complete an eligible Growvelt Learning course, then issue your certificate from the completed course page.</p></div><div className="learner-empty-highlight"><span aria-hidden="true">◇</span><div><strong>Earn trusted proof</strong><p>Complete all required text, video, and quiz activities to unlock an eligible certificate.</p></div></div><div className="certificate-space-actions"><Link className="button button-primary" href="/dashboard/my-learning">Go to My Learning</Link><Link className="text-link" href="/dashboard/explore">Explore courses <span aria-hidden="true">→</span></Link></div></section>;
