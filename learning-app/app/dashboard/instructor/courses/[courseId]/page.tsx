...
export default async function DashboardInstructorCourseEditorPage({ params }: { params: Promise<{ courseId: string }> }) {
  ...
  return <section className="course-editor-page section-shell"><header className="course-editor-heading"><div><p className="eyebrow">Course metadata</p><h1>{course.title}</h1><p>URL: <code>/{course.slug}</code></p></div><div className="course-editor-status"><CourseStatusBadge status={course.status} /><span>Updated {updatedAt}</span></div></header><CourseDraftForm mode="edit" courseId={course.course_id} status={course.status} initialValues={{ title: course.title, summary: course.summary ?? "", description: course.description ?? "", category: course.category ?? "Data Analytics", level: course.level ?? "Beginner", is_free: course.is_free, price_amount: course.price_amount, price_currency: course.price_currency }} /><aside className="course-editor-next"><p className="eyebrow">Course structure</p><h2>{course.status === "draft" ? "Build the curriculum" : "Curriculum is read-only"}</h2><p>{course.status === "draft" ? "Add modules and video or text lessons before you submit this course for review." : "This course is not a draft, so its curriculum cannot be changed by the Instructor."}</p><Link className="button button-primary" href={`/dashboard/instructor/courses/${course.course_id}/curriculum`}>{course.status === "draft" ? "Edit curriculum" : "View curriculum"}</Link></aside>{course.status === "draft" ? <CourseSubmissionForm courseId={course.course_id} /> : course.status === "pending_review" ? <aside className="course-submission-panel course-submission-waiting"><p className="eyebrow">Pending review</p><h2>Growvelt is reviewing this course.</h2><p>This course is read-only until a future review decision returns it to draft or publishes it. We do not provide a review ETA.</p></aside> : null}</section>;
}
