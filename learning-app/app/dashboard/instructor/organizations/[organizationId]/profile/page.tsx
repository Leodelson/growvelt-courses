...
export default async function ManageProviderProfilePage({ params }: { params: Promise<{ organizationId: string }> }) {
  ...
  return <section className="provider-manager-page section-shell">
    ...
    <section className="provider-manager-identity-card">
      ...
      <div className="provider-manager-actions">{profile && verification?.status === "verified" && <Link className="button button-secondary provider-manager-preview" href={`/providers/${organization.slug}`} target="_blank"><Eye aria-hidden="true" size={17} />Preview provider profile</Link>}<Link className="button button-primary" href={`/dashboard/instructor/organizations/${organizationId}/insights`}>Provider insights</Link></div>
    </section>
