...
export default async function ManageProviderProfilePage({ params }: { params: Promise<{ organizationId: string }> }) {
  ...
  return <section className="provider-manager-page section-shell">
    ...
    <section className="provider-manager-identity-card">
      ...
      {profile && verification?.status === "verified" && <Link className="button button-secondary provider-manager-preview" href={`/providers/${organization.slug}`} target="_blank"><Eye aria-hidden="true" size={17} />Preview provider profile</Link>}
    </section>
