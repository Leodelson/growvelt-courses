...
export default async function ManageProviderProfilePage({ params }: { params: Promise<{ organizationId: string }> }) {
  ...
  return <section className="provider-manager-page section-shell">
    <header className="provider-manager-top"><div><p className="eyebrow">Provider profile</p><h1>Manage {organization.name}</h1><p>Prepare your provider identity, logo, cover, and public details here. Nothing is public until Learning Admin approves your verification.</p></div><Link className="button button-secondary" href="/dashboard/profile">Back to your profile</Link></header>
    ...
    <section className="provider-manager-identity-card">
      ...
      {profile && verification?.status === "verified" && <Link className="button button-secondary" href={`/providers/${organization.slug}`} target="_blank">Preview public profile</Link>}
    </section>
    <section className="provider-manager-editor"><header><p className="eyebrow">Provider details</p><h2>Tell learners about your provider</h2><p>Required fields are marked with an asterisk. Save a draft any time; it will be public only after verification is approved.</p></header><div className="provider-manager-editor-grid"><OrganizationProfileForm organizationId={organizationId} organizationSlug={organization.slug} profile={profile ?? undefined} isVerified={verification?.status === "verified"} /><aside className="provider-manager-completion"><div className="provider-manager-completion-head"><h3>Profile completion</h3><strong>{completion}%</strong></div><div className="provider-manager-progress" aria-hidden="true"><span style={{ width: `${completion}%` }} /></div><p>Complete these details so learners can understand the provider behind each course.</p><ul>{checklist.map((item) => <li key={item.label} className={item.complete ? "is-complete" : ""}><span aria-hidden="true">{item.complete ? "✓" : "○"}</span>{item.label}</li>)}</ul></aside></div></section>
