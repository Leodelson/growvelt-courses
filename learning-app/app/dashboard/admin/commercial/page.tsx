...
import { listCommercialEarningsReleaseRuns, listCommercialOperations, listInstructorPayoutProfiles } from "@/app/lib/admin/commercial-operations";
...
export default async function AdminCommercialPage() {
  ...
  if (!user) return null;
  const [operations, releaseRuns, payoutProfiles] = await Promise.all([listCommercialOperations(user.id), listCommercialEarningsReleaseRuns(user.id), listInstructorPayoutProfiles(user.id)]);
